...
const navItems = [
  ...
  { icon: BarChart3, label: "Licenses", path: "/client/licenses" },
...
export default function ClientLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-64 flex-col border-r border-border bg-card p-4 shrink-0">
        {nav}
      </aside>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 glass h-14 flex items-center justify-between px-4">
        ...
        <div className="relative">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full gradient-accent text-[10px] flex items-center justify-center text-accent-foreground font-bold">
              {unreadCount}
            </span>
          )}
        </div>
...
function CompareBadge() {
  ...
  return (
    <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded-full gradient-primary text-primary-foreground font-bold">
      {count}
    </span>
  );
}
